...
Настройки модулей
------------
Настройки модуля задаются в файле `./config/<имя_модуля>/<имя_профиля>/settings.ini`.<br><br>Вы можете создавать дополнительные профили настроек модулей, например скопировав каталог `./config/<имя_модуля>/<имя_профиля>` в `./config/<имя_модуля>/new_<имя_профиля>` и изменив в нём настройки модуля или шаблоны конфигурации по своему усмотрению. Имя рабочего профиля задаётся в файле `./config/<имя_модуля>/module.ini`.<br><br>Для применения любых изменений необходимо перезапустить модуль либо саму программу если модуль не имеет этой возможности.
