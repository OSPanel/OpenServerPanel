...
------------
    ...
    │   │   │   └── settings.ini  # Файл настроек модуля для профиля Default
    │   │   └── module.ini        # Файл базовых настроек модуля (вкл/выкл + имя профиля)
    │   └── program.ini           # Файл настроек программы
    ├── data                      # Служебные файлы модулей (данные)
    ├── home                      # Пользовательские проекты (домены)
    ├── modules                   # Модули
    │   └── module...             # Основной каталог модуля
    │       └── ospanel_data      # Служебный каталог программы
    ├── system                    # Служебные файлы программы
    │   └── lang                  # Языковые файлы
    └── user                      # Пользовательские данные

Установка и использование
------------
На время тестов Beta-версии дистрибутив распространяется в виде самораспаковывающегося RAR-архива. Скачайте дистрибутив, распакуйте его в выбранный каталог (желательно в корень диска). Перед началом работы снимите защиту (есть есть) с файла HOSTS в настройках вашего файрвола/антивируса, а также выполните настройку необходимых исключений для исполняемых файлов.
...
После запуска панели управления изучите справку по команде `sysprep` и выполните подготовку системы с помощью инструмента System Preparation Tool, после чего перезагрузите компьютер. Теперь программа готова к использованию, вы можете активировать необходимые модули.

В случае сбоя в своей работе программа автоматически отправит отчёт об ошибке на сервер проекта.
