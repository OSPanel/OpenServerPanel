...
После запуска панели управления изучите справку по команде `sysprep` и выполните установку необходимых для работы Open Server Panel библиотек и настройку системы, после чего перезагрузите компьютер. Теперь программа готова к использованию, вы можете активировать необходимые модули. В случае возникновения ошибки программа автоматически отправит отчёт об ошибке на сервер проекта. Не забудьте снять защиту с файла HOSTS (есть есть) в настройках вашего файрвола/антивируса и добавить программу + модули в исключения.
...
| :information_source: ВАЖНО: Для применения изменений необходимо перезапустить программу. |
| --- |
...
<br><br>
| :information_source: ВАЖНО: Для применения любых изменений необходимо перезапустить модуль или саму программу (если модуль не имеет этой возможности). |
| --- |
