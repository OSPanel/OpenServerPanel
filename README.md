...
Настройки модулей
------------
Настройки модуля задаются в файле `./config/<имя_модуля>/<имя_профиля>/settings.ini`.<br><br>Вы можете создать новый профиль скопировав папку `./config/<имя_модуля>/<имя_профиля>/settings.ini` в `./config/<имя_модуля>/new_<имя_профиля>/settings.ini` и изменив в ней настройки модуля или шаблоны конфигурации по своему усмотрению. Имя рабочего профиля задаётся в файле `./config/<имя_модуля>/module.ini`.<br><br>Для применения любых изменений необходимо перезапустить модуль либо саму программу если модуль не имеет этой возможности.
