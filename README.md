...
| `lang` | Язык программы. Допустимые значения языка: (см. содержимое `./langs`). Можно использовать значение `auto`, в этом случае будет использован системный язык, а при отсутствии языкового файла системного языка - английский. |
