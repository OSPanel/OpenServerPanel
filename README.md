...
Настройки модулей
------------
Настройки модуля задаются в файле `./config/<имя_модуля>/<имя_профиля>/settings.ini`.<br><br>Вы можете создать новый профиль скопировав папку `./config/<имя_модуля>/<имя_профиля>/settings.ini` в `./config/<имя_модуля>/new_<имя_профиля>/settings.ini` и изменив в ней настройки модуля или шаблоны конфигурцаии по своему усмотрению. Имя рабочего профиля задаётся в файле `./config/<имя_модуля>/module.ini`.<br><br>Для применения любых изменений необходимо перезапустить модуль либо саму программу если модуль не имеет этой возможности.
...
Секции этого типа описывают работу с файлами конфигурации модуля (кол-во секций не ограничено). Шаблоны конфигурации модуля должны находятся в каталоге `./config/<имя_модуля>/<имя_профиля>/templates`. В процессе запуска модуля шаблоны описанные в таких секциях будут преобразованы во временные рабочие файлы конфигурации.
