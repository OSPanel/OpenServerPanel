...
:warning: **Важно**: Не запускайте программу и панель управления от имени администратора! Это потенциально небезопасно и не является необходимым условием для работы Open Server Panel. Права администратора потребуются только при запуске System Preparation Tool (команда `sysprep` запросит нужные права после вызова).
