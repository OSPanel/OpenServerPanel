...
Настройки модулей
------------
Настройки модуля задаются в файле `./config/<имя_модуля>/<имя_профиля>/settings.ini`. По умолчанию используется профиль `Default`. Имя рабочего профиля задаётся в файле `./config/<имя_модуля>/module.ini`.<br><br>Резервная копия настроек и конфигурации модуля хранится в каталоге `./modules/<имя_модуля>/ospanel_data/original` и в будущем будет использоваться в панели управления для функции восстановления оригинальной конфигурации.<br><br>Вы можете создавать дополнительные профили настроек для модулей. Для создания нового профиля, например `MyProfile`, достаточно скопировать каталог `./config/<имя_модуля>/Default` в `./config/<имя_модуля>/MyProfile` и изменить в нём настройки/шаблоны по своему усмотрению.<br><br>:warning: **Важно**: Для применения любых изменений необходимо перезапустить модуль или саму программу (если модуль не имеет этой возможности).
