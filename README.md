...
:warning: **Важно**: Не запускайте программу и панель управления от имени администратора! Права администратора требуются только при запуске System Preparation Tool (команда `sysprep` запросит нужные права после вызова). Работа с правами администратора потенциально небезопасна и не является необходимым условием для работы Open Server Panel.
