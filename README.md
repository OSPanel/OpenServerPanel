...
| `lang` | Язык программы. Допустимые значения языка: (см. содержимое `./langs`).<br><br>Можно использовать значение `system`, в этом случае будет использован системный язык, а при отсутствии языкового файла системного языка - английский. |
