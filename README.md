...
Настройки программы
------------
Настройки программы и модулей (глобальные опции) задаются в файле `config/program.ini`. Для применения изменений необходимо перезапустить программу.
...
### Секция `[environment]` ###
Секция служит для управления переменными общей среды окружения. Заданные здесь переменные применяются ко всем модулям, к запускаемым процессам и к окружению самой программы, но не влияют на переменные среды окружения Windows. Для любого модуля эти переменные можно переопределить/дополнить/удалить в файле его настроек `./config/<имя_модуля>/<имя_профиля>/settings.ini`. Для удаления переменной из среды окружения задайте пустое значение либо используйте глобальный фильтр (см. `allowed_env_vars`). Добавьте в конец `PATH` следующую запись если планируете использовать PowerShell: `;%SYSTEMROOT%\System32\WindowsPowerShell\v1.0\`
...
Настройки модулей
------------
Настройки модуля задаются в файле `./config/<имя_модуля>/<имя_профиля>/settings.ini`. По умолчанию используется профиль `Default`. Резервные копии файла настроек и файлов конфигурации модуля хранятся в каталоге `./modules/<имя_модуля>/original` и в будущем будут использоваться в панели управления для функции восстановления оригинальной конфигурации.<br><br>Вы можете создавать дополнительные профили настроек модулей, например скопировав каталог `./config/<имя_модуля>/Default` в `./config/<имя_модуля>/MyProfile` и изменив в нём настройки модуля или шаблоны конфигурации по своему усмотрению. Имя рабочего профиля задаётся в файле `./config/<имя_модуля>/module.ini`.<br><br>Для применения любых изменений необходимо перезапустить модуль либо саму программу если модуль не имеет этой возможности.
