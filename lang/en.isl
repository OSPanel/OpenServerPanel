...
CreateDesktopIcon=Create a &desktop shortcut (for all users)
...
Msvcr=Install Microsoft Visual C++ 2005-2019 Redistributable Packages
Msvcrlite=Install DLLs RuntimePack Lite
Sysopts=Optimize Windows settings (reboot)
Netopts=Optimize network stack settings (reboot)
Ssdopts=Optimize Windows settings to work with SSD (reboot)
UnblHosts=Allow write to HOSTS file (for all users)
Allusrs=For all users
Currusr=For the current user only
Runexe=Launch Open Server Panel
RunManual=User's manual
RunDonate=Donate to the project
MSVC=Microsoft required components
...
OsCoreComp=Control Panel
OsServer=Web servers
OsWTools=Web applications
OsNoSQL=NoSQL systems
OsSQL=DBMS
OsPrograms=Web development software
OsProgramsJcpicker=Just Color Picker — color picker
OsProgramsOpera=Opera — web browser
OsProgramsBrightMaster=Bright Master — monitor brightness control
OsProgramsSimplewall=Simplewall — firewall
OsProgramsSublime=Sublime Text — text editor
OsProgramsWinSCP=WinSCP — SFTP/SCP/FTP client
